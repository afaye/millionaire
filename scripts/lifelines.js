...
function phoneAFriend() {
  ...
  $('#message-text').html("Choisissez une seule personne présente autour de vous pour lui poser la question");
  // ancien appel a un ami // $('#message-text').html(message);
...
function askAudience() {
  ...
  $('#message-text').html("Il est maintenant temps pour le public de voter !");
  // ancien vote du public //$('#message-text').html("<div><div id='bar-A'></div><div id='bar-B'></div><div id='bar-C'></div><div id='bar-D'></div></div><table><tr><td>A</td><td>B</td><td>C</td><td>D</td></tr></table>");
...
function switchQuestion() {
  ...
  // ancien // readyQuestion(questionNum);
  // ONLY when the question has fully loaded, display it and hide the message
  // ancien // checking = setInterval(function(){
  //a  if (getQuestionSuccess == true) {
  //a   clearInterval(checking);
  //a    startQuestion();
  //a    $('#message-board').css('visibility','hidden');
      // this is only a late-addition lifeline: the icon pops up after Q8 and disppears after use, so hide and deactivate it
      $('#message-text').html("L'animateur va vous donner son avis...");
      $('#confirm').html('Ok').css('visibility','inherit');
      $('#quit').css('visibility','hidden');
      $('#next-question').css('visibility','hidden');
